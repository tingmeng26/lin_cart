...
try {
  ...
  if ($method == 'up') {
    $sql = "select {$colname['ind']} as ind from $table where {$colname['ind']} > :ind order by {$colname['ind']} asc";
  } else {
    $sql = "select {$colname['ind']} as ind from $table where {$colname['ind']} < :ind order by  {$colname['ind']} desc";
  }
