...
<body>
                                    ...
                                    <div class="col-md-12">
                                        <div class="form-group">
                                            <label class="col-sm-3 col-lg-4 control-label">
                                                <?php echo $fhelp->drawLabel($colname['ld_lang']) ?>
                                            </label>
                                            <div class="col-sm-3 controls">
                                                <?php echo $fhelp->drawForm($colname['ld_lang']) ?>
                                            </div>
                                        </div>

                                        <div class="form-group">
                                            <label class="col-sm-3 col-lg-4 control-label">
                                                <?php echo $fhelp->drawLabel($colname['english']) ?>
                                            </label>
                                            <div class="col-sm-3 controls">
                                                <?php echo $fhelp->drawForm($colname['english']) ?>
                                            </div>
                                        </div>

                                        <div class="form-group">
                                            <label class="col-sm-3 col-lg-4 control-label">
                                                <?php echo $fhelp->drawLabel($colname['key']) ?>
                                            </label>
                                            <div class="col-sm-3 controls">
                                                <?php echo $fhelp->drawForm($colname['key']) ?>
                                            </div>
                                        </div>

                                        <div class="form-group">
                                            <label class="col-sm-3 col-lg-4 control-label">
                                                <?php echo $fhelp->drawLabel($colname['val']) ?>
                                            </label>
                                            <div class="col-sm-3 controls">
                                                <?php echo $fhelp->drawForm($colname['val']) ?>
                                            </div>
                                        </div>
    ...
    <script type="text/javascript">
        ...
        function langKey_check() { //驗證
            var re = false;
            var ld_lang = $("#<?php echo $colname['ld_lang']; ?>").val();
            var _key = $("#<?php echo $colname['key']; ?>").val();
            if (ld_lang != "" && _key != "") {
                $.ajax({
                    url: "checkdata.php",
                    type: "POST",
        ...
        jQuery.validator.addMethod("langKey_check", function(value, element) {
            return langKey_check();
        }, "<?php coderLang::t("langdictionaryjs1"); //此國家代碼Key已被使用，請重新輸入! 
            ?>");

        $("#<?php echo $colname['ld_lang']; ?>").rules("add", {
            langKey_check: true
        });

        $("#<?php echo $colname['key']; ?>").rules("add", {
            langKey_check: true
        });
    </script>
</body>
