...
$table_ldic=coderDBConf::$lang_dictionary;
$colname_ldic=coderDBConf::$col_lang_dictionary;
...
function isDateNotExisit($type,$val,$id=0){
    ...
    switch ($type) {
        case 'lang':
            ...
            if (!$db->query_first("select {$colname['id']} from $table where `{$colname['lang']}`='".hc($val)."' $where")){
                return true;
            }else {
                return false;
            }
