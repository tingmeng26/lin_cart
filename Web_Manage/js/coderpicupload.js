...
$.fn.coderpicupload = function(settings) {
	...
	return this.each(function() {
		...
		function init(){
            ...
            var click_num = "";
            if(_settings.click_num != null)
            {
                var click_num = _settings.click_num;
            }
			$hidden=$('<input type="hidden" thisid="'+_settings.id+'" name="'+_settings.id+'" id="'+_settings.id+click_num+'" class="hasvalidate" value="" '+(_settings.required==true ? ' required="yes" ' : ' ')+'>');
			$buttoncontent.append($hidden);

			if(_settings.id_click_num!=null)
			{
				$hidden_click_num=$('<input type="hidden" thisid="'+_settings.id+'" name="'+_settings.id_click_num+'" id="'+_settings.id_click_num+'" class="hasvalidate" value="" '+(_settings.required==true ? ' required="yes" ' : ' ')+'>');
				$buttoncontent.append($hidden_click_num);
				    ...
				    _thisokbtn.click(function(event) {
				    	if(Math.floor($('#jcrop_w').val()) || Math.floor($('#jcrop_h').val())){
				    		if(Math.floor($('#jcrop_w').val())<_settings.pics[_sno].width || Math.floor($('#jcrop_h').val())<_settings.pics[_sno].height){
				    			...
				    			//return false;
				    			if(!confirm("建議圈選範圍需大於 "+_settings.pics[_sno].width+"X"+_settings.pics[_sno].height+"\n是否確定裁切? (尺寸不足部分將自動補白底)") ){
				    				return false;
				    			}
				    		}
				    		$.ajax({
			...
			$file.change(function() {
				...
				if(pic){
					...
					xhr.onload = function() {
						...
						$process.fadeOut(1500,_settings.upload_callback);
						if(r['result']==true){
							showPics(r);
					...
					fd.append('pic',pic);
					fd.append('path', _settings.filepath);
		...
		function showPics(pic,croptag){
			...
			if(_settings.id_click_num!=null){$("#"+_settings.id_click_num).val(pic['filename']);}
			var pics=_settings.pics;
			if(pics!=null){
				var _pics;
				for(var i=0;i<pics.length;i++){
		...
		function clearPics(){
			...
			$hidden.val('');
			if(_settings.id_click_num!=null){$("#"+_settings.id_click_num).val('');}
			_settings.croptagary = {};
			var pics=_settings.pics;
			if(pics!=null){
				for(var i=0;i<pics.length;i++){
