	...
	$.fn.megaselectlist = function (settings) {
		...
		return this.each(function () {
			...
			if (optgroups.length > 0) {
			    ...
			    var optionsgroupnum=config.optionsgroupnum;
				options = $This.children("option");
				
				var optionlength = Math.ceil(((options.length))/optionsgroupnum);
				var optionmuchlength = ((options.length))%optionsgroupnum;
				var endnum=0;
				for (var og = 1; og <= optionsgroupnum; og++) {
					replacementHtml += '<div class="' + classModifier + 'column"><ul>';
					var tabindex = "0";
					if (config.animate) {
						tabindex = "-1";
					}
					if(optionmuchlength < og){endnum = 1};
					   for (i = 0+(optionlength*(og-1)); i < (optionlength*og)-endnum; i++) {
						   var x = IsSelected(selectedOptionValues, $(options[i]).val());
						   var cnn = "";
						   if (x) {
						    	cnn = ' class="currentitem"';
			...
			replacementHtml += '<div style="clear: both;"></div></div>' +
				'<div style="display: none;"><select name="' + originalName + '" id="' + originalId + '" multiple="true" size="5"></div>';
			...
			$("#" + classModifier + nextModifierNumber + " li").click(function () {
				$(this).parent().parent().parent().parent().find("p:first").html("<span></span>");
                //$(".megaselectlist>p").html("<span></span>");
				//alert($(".megaselectlist>p").html())
				var item = $(this);
				...
				if(spanText==""){$(this).parent().parent().parent().parent().find("p:first").html("請選擇:<span></span>");}
				
				return false;
			});
			$(".megaselectlistcolumn").width(100/optionsgroupnum+"%");
			if (config.animate) {
