      ...
      pre: function (_this) {
          if (_this.parent().parent().prev().length > 0) {
              $.each(orderSelf, function (key, value) {
                  ...
                  _this.parent().parent().find('.order_num').next().after(pichtml_prev);
                  _this.parent().parent().prev().find('.order_num').next().after(pichtml);
      ...
      next: function (_this) {
          if (_this.parent().parent().next().length > 0) {
              $.each(orderSelf, function (key, value) {
                  ...
                  _this.parent().parent().find('.order_num').next().after(pichtml_next);
                  _this.parent().parent().next().find('.order_num').next().after(pichtml);

              });
          }

      },
      getReset:function(){ //重置
          click_num=1;
      ...
      getInput: function (ind, $td, obj, val, click_num, thisdivid, selectBoxManipulation_oi, bind_obj) {
          ...
          if (type == "pic") {
              ...
              if (click_num > defaultcount || defaultcount == 0) {
                  ...
                  $td.append('<div class="form-group"><input type="hidden" style="width:100%" name="' + thisdivid + obj.key + click_num + '" id="' + thisdivid + obj.key + click_num + '" thisid="' + obj.key + '" tag="' + obj.name + '" class="hasvalidate" value="' + val + '">' + (incary[click_num] != undefined ? incary[click_num] : '')+ '</div>');
              }
          }
