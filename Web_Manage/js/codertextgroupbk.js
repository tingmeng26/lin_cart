...
CODER.Util.TextGroup=function(_id,_title,_rowid,_option,readonly,_orderColumn,_showordernum) {
    var id = _id;
    var title = _title;
    var rowid = _rowid;
    var option=_option;
    ...
    return {
            ...
            if (max==-1 || max > 0 && parent.find("tr.content").length < max) {
				...
				for (var i = 0, c = option.length; i < c; i++) {
                    ...
                    if (obj) {
                        ...
                        }else if(option[i].type=='label_ary'){
                            val = new Array();
                            //console.log(obj);
                            for(var sbmkey in option[i].key){
                                for(var sbmkeyin in option[i].key[sbmkey]){
                    ...
                    for(oi=0,count = (option[i].type=='selectBoxManipulation'?option[i].key.length:1);oi<count;oi++){
                        ...
                        this.getInput($str.index(), $td, option[i], val,click_num,id,oi);
                        $str.append($td);
                    }
                }
        ...
        getInput: function (ind, $td, obj, val,click_num,thisdivid,selectBoxManipulation_oi) {
			var validate="";
            var multi_language="";
			var placeholder="";
            var type = "";
            ...
            if(obj.upload_callback){upload_callback = obj.upload_callback}
            if (obj.type)
            {
                type = obj.type;
            }else {
            ...
            if (type == "pic") {
                var file_path = "",
                    ...
                    s_height ='100px',
                    pics = [];
				if(obj.file_path){var file_path = obj.file_path;}
                if(obj.spic_size_w){var spic_size_w = obj.spic_size_w;}
                if(obj.spic_size_h){var spic_size_h = obj.spic_size_h;}
                if(obj.s_width){var s_width = obj.s_width;}
                if(obj.s_height){var s_height = obj.s_height;}
                if(obj.pics){ var pics = obj.pics;}else{pics = [{name:'縮圖',type:6,tag:'s',width:spic_size_w,height:spic_size_h}];}
                ...
                if (obj.readonly == true) {
                    ...
                    setTimeout(function () {
                        $('#' + id + click_num).coderpicupload({ id: obj.key, pics: pics, width: '100', height: '100',required: required_o , org_width: '50px', org_height: '50px', org_pic: org_pic,filepath:file_path, position: 'right', tag: obj.name,s_width:s_width,s_height:s_height,upload_callback:upload_callback,jcrop:false,click_num:click_num});
                    }, 100);
                }
            }
                ...
                if (obj.readonly == true) {
                    ...
                    setTimeout(function () {
                        $('#' + id).coderpicupload({ id: obj.key, pics: pics, width: '100', height: '100',required: required_o , org_width: '50px', org_height: '50px', org_pic: org_pic,filepath:file_path, position: 'right', tag: obj.name,s_width:s_width,s_height:s_height,upload_callback:upload_callback,jcrop:false,id_click_num:obj.key+'_click_'+click_num});
                    }, 100);
                }
                ...
                if(validate != ""){
                    for(var key in validate){
                        for(var i in validate[key]){
                           ...
                           if(i=="html" && validate[key][i]=="yes"){thisclass += " ckeditor ";}
                        }
                    }
                }
                if(multi_language != ""){
            ...
            }else if(type == "label_nosql"){
                $td.append('<div class="form-group">' + (obj.ary_name!=""?obj.ary_name[val]:val) +'</div>');

            }else if(type == "label_ary"){

                var i;
                var list = "";
                for (i = 0; i < obj.key.length; ++i) {
                    list += '<input type="hidden" style="width:100%" name="' +thisdivid+obj.key[i]+click_num + '" id="' +thisdivid+obj.key[i]+click_num +'" thisid="'+ obj.key[i]+ '" tag="' + obj.name + '" class="hasvalidate" value="' + val[obj.key[i]] +'">'+ (list!=""?' / <br>':'') + val[obj.key[i]];

                }

                $td.append('<div class="form-group">'+ list +'</div>');
            }
            else {
                var readonly_attr =obj.readonly==true ?    " readonly='true' " : "" ;
                if (type == "num") {
